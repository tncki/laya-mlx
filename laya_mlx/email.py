...
# `from laya_mlx.email import email_questions` is a path callers already have.
from .presets import email_questions  # noqa: F401

_QUOTE_HEADERS = [
    re.compile(r"^\s*On .{0,300}wrote:\s*$", re.I),
    # "Em resposta ao que você escreveu:" is body text; a client's attribution always carries
    # a date
    ...
    re.compile(
        r"^\s*-{2,}\s*(Mensagem (original|encaminhada)|Mensaje (original|reenviado))\s*-{2,}",
        re.I,
    ),
...
# Mobile and mail-app footers. Only a line that is nothing *but* the footer matches -- "Enviado do
# meu celular o comprovante ontem." is a request -- and such a line may run to 60 characters, since
# Samsung's default ("Enviado do meu smartphone Samsung Galaxy.") is longer than a sign-off's 40.
_DEVICE = (
    r"iphone|ipad|android|ios|celular|telemóvel|móvil|galaxy|smartphone|samsung|tablet|"
...
_DISCLAIMER = re.compile(
    ...
    # Portuguese/Spanish: tied to "this message/e-mail" rather than the bare word
    # `confidencial`, which a sender's own request ("preciso do contrato confidencial") uses
    # just as often
    r"\b(esta|este) (mensagem|e-?mail|mensaje|correo)\b[^.]{0,80}"
    r"(confidencia|sigilos|privilegiad)|"
    ...
    r"\bantes de imprimir\b[^.]{0,100}"
    r"(meio ambiente|medio ambiente|natureza|planeta|realmente necess)|"
...
def clean_email_body(body: str, max_chars: int = 3000) -> str:
    ...
    for i, line in enumerate(src):
        ...
        if (
            lines
            and _HEADER_FROM_NAME.match(line)
            and i + 1 < len(src)
